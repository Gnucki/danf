    ...
    for (var i = 0; i < operations.length; i++) {
        (function(operation) {
            var processor = getProcessor.call(self, operation[0]),
                args = processor.parseArguments(operation.slice(1))
            ;

            pipe.push(function(context, meta) {
                return processor.process(context, args, meta);
            });
        })(operations[i]);
    }

    return function(context) {
        var resolvingContext = utils.clone(context),
            originalContext = resolvingContext,
            resolvingMeta = {}
        ;

        for (var i = 0, length = pipe.length; i < length; i++) {
            resolvingContext = pipe[i](
                resolvingContext,
                resolvingMeta
            );
        }
