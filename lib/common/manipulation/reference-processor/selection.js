 ...
 * @inheritdoc
 */
Selection.prototype.parseSpecificParameters = function(serializedParameters) {
    var self = this,
        methods = [],
        match = serializedParameters.match(
            /(^(?:[^-+/*%\(]*(?:\([^\)]*\)*))*[^-+/*%\(]*)([-+/*%].*)$/
        ),
        operation = match && match[2],
        segments = (match && match[1] || serializedParameters)
...
Selection.prototype.processContext = function(context, parameters, stream) {
    ...
    return parameters.operation
        ? this.evaluateOperation(
            '{0}{1}'.format(value, parameters.operation),
            stream,
            context
        )
        : value
